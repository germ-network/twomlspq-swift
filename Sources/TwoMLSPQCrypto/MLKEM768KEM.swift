...
/// that provider's `MLKEM768` (swift-crypto) usage exactly (same archival
/// format), so it inherits the same cross-runtime reconstruction guarantee
/// and the same `@available` floor.
...
public enum MLKEM768KEM {
	...
	/// Sendable) via `MLKEM768`'s 96-byte `integrityCheckedRepresentation`.
	...
	{
		let recipient = try MLKEM768.PublicKey(rawRepresentation: ek)
