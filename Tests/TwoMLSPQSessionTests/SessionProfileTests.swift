...
@Suite struct SessionProfileTests {
	@available(iOS 26, macOS 26, *)
	private var classicalProvider: any MLS.CipherSuiteProvider {
		SessionTestSupport.classicalProvider
	}
	@available(iOS 26, macOS 26, *)
	private var pqProvider: MLKEM768CipherSuiteProvider { SessionTestSupport.pqProvider }
	...
	private func recorded(_ group: MLS.RFC9420.Group?) throws -> SessionProfile {
		try SessionProfile.recorded(in: try #require(group).context)
	}
	...
	@available(iOS 26, macOS 26, *)
	@Test func keyPackagesAdvertiseOnTheClassicalLeafOnly() throws {
		let quiet = try Principal.generate(
			clientID: Data("kp".utf8), classicalProvider: classicalProvider,
			pqProvider: pqProvider)
		let quietInvitation = try quiet.generateInvitation(lastResort: false).invitation
		let quietKP = try #require(quietInvitation.combinerKeyPackage)
		#expect(!advertises(quietKP.classical.leafNode))

		let loud = try SessionTestSupport.principal("kp-loud", profile: .correct)
		let loudInvitation = try loud.generateInvitation(lastResort: false).invitation
		let kp = try #require(loudInvitation.combinerKeyPackage)
		#expect(advertises(kp.classical.leafNode))
		#expect(!advertises(kp.pq.leafNode))
		let reparsed = try #require(CombinerKeyPackage(publishedBlob: kp.publishedBlob()))
		#expect(advertises(reparsed.classical.leafNode))
	}

	/// Founding leaves follow the party's own classical key package. Kills:
	/// founding leaves not advertising in a correct session.
	@available(iOS 26, macOS 26, *)
	@Test func foundingLeavesFollowTheOwnKeyPackage() throws {
	...
	@available(iOS 26, macOS 26, *)
	@Test func bothAdvertiseRecordsTheCorrectProfileOnBothClassicalHalves() throws {
		var (alice, bob) = try SessionTestSupport.establishedAndExchanged(profile: .correct)
		#expect(try recorded(alice.sendGroup?.classical) == .correct)
		#expect(try recorded(bob.recvGroup?.classical) == .correct)
		#expect(try recorded(bob.sendGroup?.classical) == .correct)
		#expect(try recorded(alice.recvGroup?.classical) == .correct)
		#expect(try recorded(alice.sendGroup?.pq) == .deployedCompatible)
		#expect(alice.profile == .correct)
		#expect(bob.profile == .correct)

		let kp = try alice.pqBootstrapBegin().frame
		_ = try alice.pqBootstrapJoin(try bob.pqBootstrapRespond(kp).frame)
		#expect(try recorded(bob.sendGroup?.pq) == .deployedCompatible)
		#expect(try recorded(alice.recvGroup?.pq) == .deployedCompatible)
	}

	/// Kills: recording when only one key package advertises.
	@available(iOS 26, macOS 26, *)
	@Test func oneSidedAdvertRecordsNothing() throws {
		for (aliceProfile, bobProfile) in [
			(SessionProfile.correct, SessionProfile.deployedCompatible),
			(.deployedCompatible, .correct),
		] {
			let alicePrincipal = try SessionTestSupport.principal(
				"os-a", profile: aliceProfile)
			let bobPrincipal = try SessionTestSupport.principal(
				"os-b", profile: bobProfile)
			...
			#expect(
				try recorded(initiated.session.sendGroup?.classical)
					== .deployedCompatible)
			...
			#expect(bob.profile == .deployedCompatible)
		}
	}
	...
	private func forgedWelcomeA(
		...
		let (_, welcome) = try MLS.Combiner.CombinerGroup.establish(
			classical: try half(
				classical, peer: bob.keyPackage.classical, classicalProvider),
	...
	private func assertReceiveRejects(
		...
		#expect(
			throws: TwoMLSError.sessionProfileMismatch, sourceLocation: sourceLocation
		) {
	...
	@available(iOS 26, macOS 26, *)
	@Test func acceptorRejectsARecordItsOwnComputationDisagreesWith() throws {
		...
		assertReceiveRejects(
			...
			welcome: try forgedWelcomeA(
				alice: aliceC, bob: bobQuiet, classicalExtra: record))
		...
		assertReceiveRejects(
			...
			welcome: try forgedWelcomeA(
				alice: aliceQuiet, bob: bobC, classicalExtra: record))
		...
		#expect(throws: Never.self) {
			try TwoMLSSession.receive(
				identity: bobC3,
				welcome: try forgedWelcomeA(
					alice: aliceC3, bob: bobC3, classicalExtra: record),
	...
	@available(iOS 26, macOS 26, *)
	@Test func malformedRecordIsRejected() throws {
		...
		assertReceiveRejects(
			alice: alice, bob: bob,
			welcome: try forgedWelcomeA(
				alice: alice, bob: bob, classicalExtra: withContents))
		let twice =
			SessionProfile.correct.recordExtensions
			+ SessionProfile.correct.recordExtensions
		#expect(throws: TwoMLSError.sessionProfileMismatch) {
	...
	@available(iOS 26, macOS 26, *)
	@Test func rejectedRecordLeavesTheInvitationUnconsumed() throws {
		...
		let their = try #require(invitation.combinerKeyPackage)
		let alice = try SessionTestSupport.identity("inv-a", profile: .correct)
		let bobIdentity = try #require(invitation.identity)
	...
	@available(iOS 26, macOS 26, *)
	@Test func initiatorRejectsAReturnWelcomeThatDropsOrAddsTheRecord() throws {
		...
		for (bobProfile, forgedRecord) in [
			...
			let alicePrincipal = try SessionTestSupport.principal(
				"rw-a", profile: .correct)
			let bobPrincipal = try SessionTestSupport.principal(
				"rw-b", profile: bobProfile)
			var (invitation, _) = try bobPrincipal.generateInvitation(lastResort: true)
			let initiated = try TwoMLSSession.initiate(
				principal: alicePrincipal,
				their: try #require(invitation.combinerKeyPackage))
			...
			let founding = try TwoMLSIdentity.mintFoundingLeaf(
				...
				capabilities: TwoMLSIdentity.leafCapabilities(advertising: [
					.correct
				]))
			let (_, welcomeB) = try APQGroup.establishClassicalOnly(
				founder: MLS.Combiner.HalfCreation(
					groupID: classicalProvider.randomBytes(
						classicalProvider.hashSize),
					leafNode: founding.leafNode,
					leafSecretKey: founding.leafSecretKey,
				...
				pqGroupID: pqProvider.randomBytes(pqProvider.hashSize),
				crossPSK: crossPSK,
			...
			let forged = Frames.encodeMessageFrame(
				staple: staple, proposal: proposal, app: app)
			#expect(throws: TwoMLSError.sessionProfileMismatch, "\(profile)") {
	...
	@available(iOS 26, macOS 26, *)
	@Test func replacementLeafMustKeepAdvertising() throws {
		for profile in [SessionProfile.correct, .deployedCompatible] {
			for atFold in [false, true] {
				...
				_ = try bob.prepareToEncrypt()
				let send = try #require(bob.sendGroup).classical
				let aliceIndex = try #require(
					send.tree.nonBlankLeaves().first {
						$0.index != send.myLeafIndex
					}?.index)
				var leaf = try MLS.RFC9420.LeafNode(
					mlsEncoded: try #require(send.tree.leaf(at: aliceIndex))
						.encoded)
				...
				leaf.signature = try MLS.signWithLabel(
					...
					content: try leaf.toBeSigned(
						placement: .inGroup(
							groupID: send.context.groupID,
							leafIndex: aliceIndex)))
				let pub = try MLS.RFC9420.protectPublic(
					classicalProvider,
					content: MLS.RFC9420.FramedContent(
						groupID: send.context.groupID,
						epoch: send.context.epoch,
						sender: .member(aliceIndex),
						authenticatedData: Data(),
						content: .proposal(.update(leaf))),
					groupContext: send.context, confirmationTag: nil,
					signingKey: key,
					membershipKey: send.epoch.membershipKey)
				let message = try MLS.RFC9420.Message.publicMessage(pub)
					.mlsEncoded()
				let digest = classicalProvider.randomBytes(8)
				let offered = (
					digest: digest, proposing: Data("ka-a".utf8),
					message: message
				)
				...
				if profile == .correct {
					...
					#expect(throws: Never.self, "fold: \(atFold)") {
						try attempt()
					}
				}
				_ = alice
	...
	/// record.
	@available(iOS 26, macOS 26, *)
	@Test func pqHalfRecordIsRejected() throws {
	...
	@available(iOS 26, macOS 26, *)
	@Test func correctProfileAnnouncesNothing() throws {
		for profile in [SessionProfile.correct, .deployedCompatible] {
			...
			_ = try bob.processIncomingDecrypted(
				try alice.encrypt(Data("fold".utf8)).frame)
			...
			#expect(
				pub.content.authenticatedData
					== (profile == .correct ? Data() : bob2),
				"\(profile)")
			// The responder hint is leaf-derived, so it is the same in both.
			#expect(try alice.pqRekeyRespond(begin.frame).rotatedCredential == bob2)
		}
	}
	...
	@available(iOS 26, macOS 26, *)
	@Test func correctProfileOpensTheReciprocalAtOnce() throws {
		for profile in [SessionProfile.correct, .deployedCompatible] {
			var (alice, bob) = try RatchetTests.fullyEstablishedTurnOnBob(
				profile: profile)
			...
			_ = try alice.prepareToEncrypt()
			_ = try bob.processIncomingDecrypted(
				try alice.encrypt(Data("ack".utf8)).frame)
			_ = try bob.prepareToEncrypt()
			_ = try bob.encrypt(Data("turn".utf8))
			switch (profile, bob.pqInflight) {
			case (.correct, .some(.rekeyInitiated)),
				(.deployedCompatible, .some(.initiating)):
				break
			default:
				Issue.record(
					"\(profile): unexpected \(String(describing: bob.pqInflight))"
				)
			}
		}
	}
	...
	@available(iOS 26, macOS 26, *)
	@Test func profileSurvivesRestore() throws {
		...
		let restored = try TwoMLSSession.restore(
			...
			classicalProvider: classicalProvider, pqProvider: pqProvider)
		#expect(restored.profile == SessionProfile.correct)
	}
}
